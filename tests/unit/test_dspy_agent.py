...
@pytest.fixture
def agent() -> DSPyAgent:
    agent = DSPyAgent(llm=LLM(LLMConfig()), config=AgentConfig())
    agent.llm = Mock()
    agent.llm.config = Mock()
    agent.llm.config.max_message_chars = 100
    return agent

def test_cmd_output_observation_message(agent: DSPyAgent):
    obs = CmdOutputObservation(command='echo hello', content='Command output', command_id=1, exit_code=0)
    result = agent._get_observation_message(obs)
...
def test_ipython_run_cell_observation_message(agent: DSPyAgent):
    obs = IPythonRunCellObservation(code='plt.plot()', content='IPython output\n![image](data:image/png;base64,ABC123)')
    result = agent._get_observation_message(obs)
    assert result is not None
    assert result.role == 'user'
    assert len(result.content) == 1
...
def test_agent_delegate_observation_message(agent: DSPyAgent):
...
def test_error_observation_message(agent: DSPyAgent):
...
def test_unknown_observation_message(agent: DSPyAgent):
    obs = Mock()
