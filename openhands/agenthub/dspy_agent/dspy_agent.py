...
import openhands.agenthub.dspy_agent.function_calling as dspy_function_calling
from openhands.agenthub.dspy_agent.action_parser import DSPyResponseParser
...
class DSPyAgent(Agent):
    VERSION = '1.0'
    """
    The DSPy Agent is a coding agent that uses DSPy program optimization for improving prompts.

    ### Overview

    This agent implements the DSPy idea that consolidates LLM agents’ actions into a unified code action space for both simplicity and performance.
    ...
    def __init__(
        ...
        if self.function_calling_active:
            self.tools = dspy_function_calling.get_tools(
                dspy_enable_browsing=self.config.dspy_enable_browsing,
                dspy_enable_jupyter=self.config.dspy_enable_jupyter,
                dspy_enable_llm_editor=self.config.dspy_enable_llm_editor,
            )
            logger.debug(
                f'TOOLS loaded for DSPyAgent: {json.dumps(self.tools, indent=2)}'
            )
            ...
            self.action_parser = DSPyResponseParser()
            self.prompt_manager = PromptManager(
                microagent_dir=os.path.join(os.path.dirname(__file__), 'micro'),
    ...
    def step(self, state: State) -> Action:
        ...
        if self.function_calling_active:
            actions = dspy_function_calling.response_to_actions(response)
